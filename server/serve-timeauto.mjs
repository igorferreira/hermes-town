#!/usr/bin/env node
// Start the Hermes Town TimeAutomatico replica server.
//
//   npm run serve:live
// node server/serve-timeauto.mjs [--port 4190] [--host 127.0.0.1|localhost|::1]
...
function parseArgs(argv) {
  const options = {
    port: Number(process.env.HERMES_TOWN_PORT ?? 4190),
...
if (options.help) {
  console.log([
    'Usage: node server/serve-timeauto.mjs [options]',
    '',
    '  --port N               Listen port (default 4190)',
...
console.log('Hermes Town TimeAutomatico replica');
